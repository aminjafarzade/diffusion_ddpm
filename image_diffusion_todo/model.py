    ...
    def sample(
        ...
        for t in tqdm(self.var_scheduler.timesteps):
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred = (1 + guidance_scale)*self.network(x_t, timestep=timestep_cp, class_label=class_label_ext[len(class_label):]) - \
                    guidance_scale*self.network(x_t, timestep=timestep_cp, class_label=class_label_ext[:len(class_label)])
                #######################
            else:
