...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        sigmas = self._get_teeth(self.sigmas, t)
        alphas = self._get_teeth(self.alphas, t)
        alphas_cumprod = self._get_teeth(self.alphas_cumprod, t)

        sample_prev = 1 / alphas.sqrt() * (x_t - (1 - alphas) / (1 - alphas_cumprod).sqrt()
                                           * eps_theta) + sigmas * eps
        #######################
    ...
    def add_noise(
        ...
        alphas_prods = self._get_teeth(self.alphas_cumprod, t)
        x_t = torch.sqrt(alphas_prods) * x_0 + \
            torch.sqrt(1 - alphas_prods) * eps
        #######################
