...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        xt = torch.sqrt(alphas_prod_t) * x0 + \
            torch.sqrt(1 - alphas_prod_t) * noise
    ...
    def p_sample(self, xt, t):
        ...
        # print(t.item())
        var = torch.sqrt(extract(self.var_scheduler.betas, t, xt))
        ...
        x_t_prev = (1 / alpha_t.sqrt())*(xt - eps_factor *
                                         eps_theta) + eps * var
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        if t_prev >= 0:
            alpha_prod_t_prev = extract(
                self.var_scheduler.alphas_cumprod, t_prev, xt)
        else:
            alpha_prod_t_prev = torch.ones_like(alpha_prod_t).to(self.device)

        var = eta * torch.sqrt((1 - alpha_prod_t_prev) /
                               (1 - alpha_prod_t)) * torch.sqrt(1 - alpha_prod_t/alpha_prod_t_prev)
        eps_theta = self.network(xt, t)

        x_t_prev = torch.sqrt(alpha_prod_t_prev) * \
            ((xt - (1 - alpha_prod_t).sqrt()*eps_theta) /
             torch.sqrt(alpha_prod_t)) + (1 - alpha_prod_t_prev - var**2).sqrt()*eps_theta + var*torch.randn_like(xt)
    ...
    def ddim_p_sample_loop(self, shape, num_inference_timesteps=50, eta=0.0):
        ...
        xt = torch.randn(shape).to(self.device)
        # some int to tensor problems maybe
    ...
    def compute_loss(self, x0):
        ...
        t = (
            torch.randint(
                0, self.var_scheduler.num_train_timesteps, size=(batch_size,))
            .to(x0.device)
            .long()
        )
