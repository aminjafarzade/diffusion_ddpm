...
class SimpleNet(nn.Module):
    def __init__(
        ...
        self.simple_net = nn.ModuleList([])

        for i in range(len(dim_hids)):
            if i == 0:
                self.simple_net.append(TimeLinear(
                    self.dim_in, self.dim_hids[i], self.num_timesteps))
            else:
                self.simple_net.append(
                    TimeLinear(self.dim_hids[i - 1], self.dim_hids[i], self.num_timesteps)
                )

        self.simple_net.append(TimeLinear(
            self.dim_hids[-1], self.dim_out, self.num_timesteps))
        self.relu = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        ######## TODO ########
        # DO NOT change the code outside this part.
        for i in range(len(self.simple_net)):
            x = self.simple_net[i](x, t)
            if i != len(self.simple_net) - 1:
                x = self.relu(x)
        ######################
        return x
